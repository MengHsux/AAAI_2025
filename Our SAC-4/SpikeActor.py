...
from SACUU.utils import make_paraset
...
class SpikeActor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action, hidden_sizes=[256, 256], encoder_pop_dim=10,
                 decoder_pop_dim=10,
                 mean_range=(-3, 3), std=math.sqrt(0.15), spike_ts=10,
                 device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), to_spike='regular',
                 ntype='LIF'):
        """
        :param obs_dim: observation dimension
        :param act_dim: action dimension
        ...
        self.snn = SpikeMLP(state_dim * encoder_pop_dim, action_dim * decoder_pop_dim, hidden_sizes, spike_ts, device,
                            ntype)
        # self.decoder = PopSpikeDecoder(action_dim, decoder_pop_dim)

        self.mu = PopSpikeDecoder(action_dim, decoder_pop_dim)
        self.log_std = PopSpikeDecoder(action_dim, decoder_pop_dim)
    ...
    def forward(self, obs):
        ...
        a = self.snn(in_pop_spikes, batch_size)
